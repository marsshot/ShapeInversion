...
\subsection{Mumford-Shah}
%\textbf{Example} 
Our first example illustrates the ability of the Mumford-Shah inversion technique to segment the Marmousi model into a large number of shapes and interfaces.  Strictly speaking, no inversion is performed here.  This example also highlights how the spatial variation of the recovered parameters within each shape depends on the value of $\alpha$ in equation (\ref{eq:MS}).
...
\end{comment}
%\textbf{Example}  
Figure~\ref{fig:201a} illustrates the permeability field used to generate the pressure profile found in Figure~\ref{fig:201b} according to equation (\ref{eq:onephase}) and a Dirichlet boundary condition 
...
\caption{Permeability fields resulting from two conventional cell-based inversion techniques, (a) one without and (b) the other with a regularization term.}
\label{fig:203}
\end{figure}
}
...
\caption{(a) The permeability field and (b) the interface set $\phi_\epsilon$ obtained from the Mumford-Shah-based inversion technique.}
\label{fig:205}
\end{figure}
}
...
\caption{The anistropic permeability fields (a) $\kappa_{xx}$ and (b) $\kappa_{zz}$, and the pressure field obtaibed using equation (\ref{eq:onephase}) and permeability fields from (a) and (b).}
\label{fig:208}
\end{figure}
}
...
\caption{The anistropic permeability fields (a) $\kappa_{xx}$ and (b) $\kappa_{zz}$ resulting from the conventional cell-based inversion.}
\label{fig:211}
\end{figure}
}
...
\caption{The permeability tensor components (a) $\kappa_{xx}$, (b) $\kappa_{zz}$, and (c) the interface set $\phi_\epsilon$ resulting from the Mumford-Shah-based inversion.}
\label{fig:213}
\end{figure}
}
\sendatend{
...
\caption{(a) The background velocity model as the initial guess, and the velocity models inverted using (a) the conventional cell-based inversion and (b) Mumford-Shah inversion, and (d) the boundary set obtained from Mumford-Shah inversion. }
\label{fig:304}
\end{figure}
}

The available data in this example is the first-arrival times associated with the 501 sources and 501 receivers, as illustrated in Figure~\ref{fig:301c}.  First-arrival travel times T are computed by solving the Eikonal equation
...
\caption{(a) A modified Marmousi model with two salt geobodies and (b) an initial guess without presences of geobodies.}
\label{fig:701}
\end{figure}
}

%\textbf{Example} 
Our final Mumford-Shah example differs from the previous ones in three important ways.  First, in each of our previous examples the subsurface geomery consisted of a small number of shapes.  In this example, we consider a subsurface region possessing a large number of layers and two anomalous bodies. Second, in this example we utilize pressure data arising from the acoustic wave approximation.  Third, our final example highlights the potential uplift from annealing strategies applied to Mumford-Shah.  
...
Pressure measurements $p$ are computed by solving the acoustic wave equation with the free boundary conditions on the top surface and the absorbing boundary conditions along the other boundaries.  
\begin{align}
...
Our remaining three examples focus on phase-field inverison.  In each example, we seek to determine the location of an anomalous homogeneous body in a 4-km deep by 20-km wide subsurface.  Our first example determines the anomaly's location using first-arrival travel time data; our second example determines the anomaly's location only using gravity data; and in our third example we recover the anomaly's location using travel time data and gravity data. 
...
Travel-time (or first breakthrough time) data is generated using the velocity model illustrated in Figure~\ref{fig:301a}.  Gravity data is generated using the subsurface density model illustrated in Figure~\ref{fig:501a}.  The model assumes a uniform background density and an anomalous rectangular region with $-20 g/cm^3$ density difference relative to the background, as shown in Figure~\ref{fig:501a}. Note that the density and velocity models share the same boundary set, as can be observed from Figures~\ref{fig:301a} and~\ref{fig:501a}.  
...
Each phase-field inversion optimizes equation (\ref{eq:pf_epsilon}) using the representation of $\kappa$ given by equation (\ref{eq:pf_kappa}).  Each cell-by-cell inversion optimizes equation (\ref{eq:objective}).  All of the inversions are performed on a regular Cartesian mesh using fifty iterations of inexact Gauss-Newton. 

%\textbf{Example} 
Our first example attempts to determine the location of the anomalous body using the first-arrival travel time data illustrated in Figure~\ref{fig:301}. The initial guess is the background wave speed profile illustrated in Figure~\ref{fig:304a}; for phase-field, this amounts to an initial guess of $\phi=-1$.  For phase-field, we assume the parameters values $\kappa_i$ in equation (\ref{eq:pf_kappa}) are fixed and ony infer for the phase-field function $\phi$ which partitions the subsurface into two shapes $\{\Omega_1,\Omega_2 \}$. The domain $\Omega_1$ represents the anomalous body and the domain $\Omega_2$ represents the surrounding background. The parameter value $\kappa_1$ represents the wave speed within the anomalous body and is fixed at 4.5 km/s. The parameter value $\kappa_2$ is fixed to be the background wave speed profile, as shown in Figure~\ref{fig:304a}.

Fifty iterations of inexact Gauss-Newton are used for both inversions.  Figure~\ref{fig:304b} illustrates the wave speed resulting from the cell-based inversion; note that no explicit regularization is used here.  Figure~\ref{fig:401b} illustrates the wave speed obtained from phase-field.  This is obtained by plugging in the optimized phase-field function $\phi_\epsilon$, illustrated in Figure~\ref{fig:401a}, into equation (\ref{eq:pf_kappa}).  Figure~\ref{fig:401a} demonstrates that the phase-field function $\phi_\epsilon$ demarks two shapes with a narrow transition region where $\phi_\epsilon$ transitions from a value of 1 to a value of -1. 
...
This example seeks to determine the location of the same anomaly as the previous example, but this time using only gravity anomaly data.  Figure~\ref{fig:501a} illustrates the true density anomaly model. 
...
For our final phase-field example, we seek to determine the location of the anomalous body using first-arrival travel time data and gravity data anomaly data. The initial guesses for wave speed and density remain unchanged from the earlier examples. 
...
This paper proposed two shape-based inversion algorithms, Mumford-Shah inversion and phase-field inversion, to determine subsurface geophysical properties and structures in a unified manner from various geophysical data sets including seismic and gravity.  In a future work, we plan to extend this framework to multiparameter FWI.  We also believe that our techniques may be applicable in other scientific areas, such as medical imaging.

%\section{ACKNOWLEDGMENTS}

\newpage
\bibliographystyle{seg} 
\bibliography{shape_inversion_submitted}
