...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.45,trim=0 25 0 0, clip]{1a.png} \label{fig:1a}}
\subfloat[]{\includegraphics[scale=0.45, trim=25 25 0 0, clip]{1b.png} \label{fig:1b}}\\
\subfloat[]{\includegraphics[scale=0.45]{1c.png} \label{fig:1c}}
\subfloat[]{\includegraphics[scale=0.45, trim=25 0 0 0, clip]{1d.png} \label{fig:1d}}
...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.6,trim=0 0 0 0, clip]{3a.png} \label{fig:3a}}
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{3b.png} \label{fig:3b}}
\caption{(a) True non-dimensional isotropic permeability field ($\kappa/\kappa_{Ref}$) and (b) the non-dimensional pressure profile ($p/p_{Ref}$) obtained from this permeability field. Note that the permeability and pressure fields along with their spatial coordinates (x and z) are non-dimensionalized for the numerical conditioning of the one-phase flow inverse problems.}
\label{fig:3}
\end{figure}
}
...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.6,trim=0 0 0 0, clip]{4a.png} \label{fig:4a}}
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{4b.png} \label{fig:4b}}
\caption{Permeability fields ($\kappa/\kappa_{Ref}$) resulting from two conventional cell-based inversion techniques, (a) one without and (b) the other with a Tikhonov regularization term.}
\label{fig:4}
\end{figure}
}

Figure~\ref{fig:4} illustrates permeability fields resulting from two conventional cell-based inversion techniques, one without regularization and the other with Tikhonov regularization.  While the cell-based inversion technique without the regularization term recovers some of the large-scale features of the original model, it also contains high-frequency artifacts and does not properly capture the smooth background permeability field. The addition of Tikhonov regularization stabilizes the cell-based inversion technique.  However, the resulting permeability field smears out the background and the boundaries of the annular region. 

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.6,trim=0 0 0 0, clip]{5a.png} \label{fig:5a}}
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{5b.png} \label{fig:5b}}
\caption{(a) The inverted non-dimensional permeability field ($\kappa/\kappa_{Ref}$) and (b) the interface set $\phi_\epsilon$ obtained from the Mumford-Shah-based inversion technique.}
\label{fig:5}
\end{figure}
}
...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{8a.png} \label{fig:8a}}
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{8b.png} \label{fig:8b}}
\caption{The inverted non-dimensional anistropic permeability fields (a) $\kappa_{xx}/\kappa_{Ref}$ and (b) $\kappa_{zz}/\kappa_{Ref}$ resulting from the conventional cell-based inversion.}
\label{fig:8}
\end{figure}
}

Figure~\ref{fig:8} illustrates the permeability tensor resulting from the cell-based inversion without regularization.  Since the cell-based inversion formulation does not enforce a spatial correlation between the inverted tensor components, the result is non-unique and unstable.  In contrast, the inverted permeability tensor based on Mumford-Shah is well-structured and the interface set is correctly identified (see Figure~\ref{fig:9}).  

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{9a.png} \label{fig:9a}}
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{9b.png} \label{fig:9b}}
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{9c.png} \label{fig:9c}}
\caption{The inverted non-dimensional permeability tensor components (a) $\kappa_{xx}/\kappa_{Ref}$, (b) $\kappa_{zz}/\kappa_{Ref}$, and (c) the interface set $\phi_\epsilon$ resulting from the Mumford-Shah-based inversion.}
\label{fig:9}
\end{figure}
}
\sendatend{
...
Figure~\ref{fig:10} compares the data misfit (on a logarithmic scale) versus the number of iterations for the above results.  Interestingly, both inversions match the data.  This highlights the utility of Mumford-Shah as a regularization scheme that enforces spatial correlations between parameters.  

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{11a.png} \label{fig:11a}} \\
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{11b.png} \label{fig:11b}}
\subfloat[]{\includegraphics[scale=0.6, trim=0 0 0 0, clip]{11c.png} \label{fig:11c}}
...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.45,trim=0 12 0 0, clip]{12a.png} \label{fig:12a}}
\subfloat[]{\includegraphics[scale=0.45, trim=12 12 0 0, clip]{12b.png} \label{fig:12b}}\\
\subfloat[]{\includegraphics[scale=0.45]{12c.png} \label{fig:12c}}
\subfloat[]{\includegraphics[scale=0.45, trim=12 0 0 0, clip]{12d.png} \label{fig:12d}}
...
The available data in this example are the first-arrival times associated with the 501 sources and 501 receivers, as illustrated in Figure~\ref{fig:11c}.  First-arrival travel times T for both data generation and inversion are computed by solving the Eikonal equation
...
The initial guess for the conventional cell-based inversion technique is the background wave speed profile illustrated in Figure~\ref{fig:12a}.  The initial guess for the Mumford-Shah inversion is the background wave speed profile and $\phi_\epsilon=1$ (i.e., $\Gamma=\emptyset$).  Fifty iterations of inexact Gauss-Newton are used for the cell-based and Mumford-Shah inversions.  

The wave speed from the cell-based inversion is illustrated in Figure~\ref{fig:12b}.  Note that no regularization is included.  The wave speed and inverted interface set $\phi_\epsilon$ from the Mumford-Shah inversion are illustrated in Figure~\ref{fig:12c} and Figure~\ref{fig:12d}, respectively.  A comparison of Figures~\ref{fig:12b} and ~\ref{fig:12c} indicates that the wave speed recovered by Mumford-Shah represents an improvement over the cell-based inversion method.

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.45, trim=0 0 0 0, clip]{13a.png} \label{fig:13a}}
\subfloat[]{\includegraphics[scale=0.45, trim=0 0 0 0, clip]{13b.png} \label{fig:13b}}
...
However the anomalous target is not fully recovered by Mumford-Shah inversion as observed from Figure~\ref{fig:12c}. To improve the Mumford-Shah inversion results, we make use of the phase-field method. We will attempt to determine the location of the same anomalous body using phase-field method with the same first-arrival travel time data illustrated in Figure~\ref{fig:11}.
...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.5,trim=0 0 0 -40, clip]{14a.png} \label{fig:14a}}
\subfloat[]{\includegraphics[scale=0.5, trim=0 0 0 0, clip]{14b.png} \label{fig:14b}}
...
Gravity data are generated using the subsurface density model illustrated in Figure~\ref{fig:14a}.  The model assumes a uniform background density and an anomalous rectangular region with $-20 g/cm^3$ density difference relative to the background, as shown in Figure~\ref{fig:14a}. In reality, the background density vary in depth, but for the simplicity of this example we assume that it is constant in depth. Both Mumford-Shah and phase-field inversion methods can be employed for any spatial distributions of inversion parameters as already demonstrated. Note that the density and velocity models share the same boundary set, as can be observed from Figures~\ref{fig:11a} and~\ref{fig:14a}.  

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.45, trim=0 0 0 0, clip]{15a.png} \label{fig:15a}}
\subfloat[]{\includegraphics[scale=0.45, trim=0 0 0 0, clip]{15b.png} \label{fig:15b}}
\caption{The inversion results from (a) cell-based inversion and (b) phase-field inversion of data provided in Figure~\ref{fig:14b}.}
\label{fig:15}
\end{figure}
}
...
Gravity measurements are made at 501 locations at 300 m above the top surface of the domain.  The gravity measurements for the target model are provided in Figure~\ref{fig:14b}. 
...
Figure~\ref{fig:15a} illustrates the density anomaly resulting from the cell-based inversion; note that no regularization term is included here.  Figure \ref{fig:15b} illustrates the density anomaly resulting from phase-field inversion.  Not surprisingly, neither the conventional cell-based inversion technique nor the phase-field inversion technique accurately locates the anomalous body due to the Laplacian smoothing present in equation~\ref{eq:gravity} and the limited measurements available above the domain.

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.45, trim=0 0 0 0, clip]{16a.png} \label{fig:16a}}
\subfloat[]{\includegraphics[scale=0.45, trim=0 0 0 0, clip]{16b.png} \label{fig:16b}}
\caption{The phase-field inversion results using joint travel-time (Figure~\ref{fig:11c}) and gravity data  (Figure~\ref{fig:14b}) for (a) the wave velocity and (b) differential density.}
\label{fig:16}
\end{figure}
}
...
In this example, we seek to determine the location of the anomalous body using both first-arrival travel time data and gravity data anomaly data. The initial guesses for wave speed (Figure~\ref{fig:11a}) and density (Figure~\ref{fig:14a}) remain unchanged from the earlier examples. 

The density and wave-speed profiles for the cell-based inversion are identical to those obtained in the previous examples since the cell-based approach does not couple density and wave speed.  Figure~\ref{fig:12b} represents the inverted wave speed profile in this case and Figure~\ref{fig:15b} represents the inverted density profile. Figure~\ref{fig:16} displays the inverted wave speed and density profiles for the joint phase-field inversion, along with the phase-field function $\phi_\epsilon$. Joint inversion greatly improves the estimation of density and provides some uplift to wave speed.

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.55, trim=0 0 0 0, clip]{17a.png} \label{fig:17a}}
\subfloat[]{\includegraphics[scale=0.55, trim=0 0 0 0, clip]{17b.png} \label{fig:17b}} \\
\subfloat[]{\includegraphics[scale=0.55, trim=0 0 0 0, clip]{17c.png} \label{fig:17c}}
\subfloat[]{\includegraphics[scale=0.55, trim=0 0 0 0, clip]{17d.png} \label{fig:17d}}
...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.45,trim=0 0 0 0, clip]{18a.png} \label{fig:18a}}
\subfloat[]{\includegraphics[scale=0.445, trim=0 0 0 -27, clip]{18b.png} \label{fig:18b}}
...
The subsurface model selected for this example is shown in Figure~\ref{fig:18a}.  It is 3.1 km deep by 17 km wide and consists of the Marmousi wave speed model with anomalies overlaid.  100 seismic sources and 500 seismic receivers are placed equidistantly on the top surface of the subsurface region from $x = 0.25$km to $16.75$km.  
...
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.45,trim=0 0 0 0, clip]{19a.png} \label{fig:19a}}
\subfloat[]{\includegraphics[scale=0.45, trim=0 0 0 0, clip]{19b.png} \label{fig:19b}}
\caption{(a) The velocity model and boundary set resulting from Mumfard-Shah inversion.}
\label{fig:19}
\end{figure}
}
In this example, we focus exclusively on the Mumford-Shah inversion.  The initial guess is the wave speed profile illustrated in Figure~\ref{fig:18b} and $\phi_\epsilon=1$ (i.e. $\Gamma = \emptyset$).  The initial guess for the wave speed captures some of the large-scale trends of the Marmousi model but does not incorporate any information regarding the anomalies.  Figures~\ref{fig:19} illustrate the inverted wave speed $\kappa(x)$ and the inverted interface set $\phi_\epsilon$. The result demonstrates the ability of Mumford-Shah to determine a large number of interfaces and shapes from acoustic waveforms.

The determination of the wave speed and the boundary set requires a multi-stage approach. In the first stage, equation~\ref{eq:epsilon} is minimized using $\beta=10^{-7}$.  In the second stage, the value of $\beta$ is decreased to $\beta=10^{-8}$.  In the third stage, the value of $\beta$ is decreased to $\beta =10^{-9}$.  The data misfit and model misfit versus the number of iterations are plotted in Figure~\ref{fig:20}. The branch points in Figure~\ref{fig:20} indicate where the value of $\beta$ is decreased.  Figure~\ref{fig:20} illustrates that  keeping $\beta$ fixed leads to a plateau in data and model misfit while a strategic decrease in $\beta$ results in a decrease in both data and model misfit.

\sendatend{
\begin{figure}
\centering 
\subfloat[]{\includegraphics[scale=0.3,trim=0 0 0 0, clip]{20a.png} \label{fig:20a}}
\subfloat[]{\includegraphics[scale=0.3, trim=0 0 0 0, clip]{20b.png} \label{fig:20b}}
